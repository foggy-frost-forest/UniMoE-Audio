    ...
    def _inner_training_loop(
        ...
        if self.is_deepspeed_enabled:
            ...
            if self.has_moe_layers:
                if 'optimizer' in self.accelerator.state.deepspeed_plugin.hf_ds_config.config:
                    logger.warning("do not use optimizer in deepspeed config when using moe !!!")
                    self.accelerator.state.deepspeed_plugin.hf_ds_config.config.pop('optimizer')
                if 'scheduler' in self.accelerator.state.deepspeed_plugin.hf_ds_config.config:
                    logger.warning("do not use scheduler in deepspeed config when using moe !!!")
                    self.accelerator.state.deepspeed_plugin.hf_ds_config.config.pop('scheduler')
